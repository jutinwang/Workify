...
const ScheduleInterviewModal = ({ candidate, jobId, onClose }) => {
  ...
  // Load employer unavailable times (preview only)
  useEffect(() => {
    const loadAvailability = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          setEvents(
            Array.isArray(data.unavailableTimes)
  ...
  // Employer sends interview request
  const handleSend = async () => {
    try {
      ...
      const res = await fetch("http://localhost:4000/interviews", {
        ...
        credentials: "include",
        body: JSON.stringify({
          jobId: jobId ?? undefined, // FIXED — no nulls
      ...
      if (!res.ok) {
        setError(data.error || "Failed to send interview request.");
        return;
      }

      alert("Interview request sent to the candidate.");
      onClose(true);
    } catch (e) {
          ...
          <section className="modal-section">
            ...
            {loading ? (
              ...
              <p>
                ...
                Go to your availability page to configure it.
              </p>
            ) : (
              <div>
                <DayPilotCalendar
          ...
          <section className="modal-section">
            ...
            <div className="form-row">
              ...
              <textarea
                ...
                rows={4}
                placeholder="e.g. This will be a 30-minute Zoom interview with two engineers."
              />
            </div>
