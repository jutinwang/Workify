...
export function reducer(state, action) {
    switch (action.type) {
        ...
        return {
            ...state,
            company: { ...state.company, socials: { ...state.company.socials, ...action.payload } },
        };
        case "SET_RECRUITER":
...
export function validateContact(ci) {
    ...
    if (ci.linkedin && !URL_RE.test(ci.linkedin)) errors.linkedin = "Use a full URL (https://...)";
    if (ci.github && !URL_RE.test(ci.github)) errors.github = "Use a full URL (https://...)";
    if (ci.portfolio && !URL_RE.test(ci.portfolio)) errors.portfolio = "Use a full URL (https://...)";
    return errors;
}
...
export function validateCompany(c) {
    ...
    if (!c.website || !URL_RE.test(c.website)) errors.website = "Valid https URL required.";
    if (c.careersUrl && !URL_RE.test(c.careersUrl)) errors.careersUrl = "Use a full URL (https://...)";
    const s = c.socials || {};
    if (s.linkedin && !URL_RE.test(s.linkedin)) errors.linkedin = "Use a full URL.";
    if (s.twitter && !URL_RE.test(s.twitter)) errors.twitter = "Use a full URL.";
    if (s.instagram && !URL_RE.test(s.instagram)) errors.instagram = "Use a full URL.";
    return errors;
}
...
export function validateRecruiter(r) {
    ...
    if (r.phone && !PHONE_RE.test(r.phone)) errors.phone = "Enter a valid phone or leave blank.";
    if (r.linkedin && !URL_RE.test(r.linkedin)) errors.linkedin = "Use a full URL.";
    return errors;
}
